...
console.log(back)
console.log('=== 是否一致 ===')
console.log(md === back ? '✅ 完全一致' : '❌ 有差异')

// 逐行对比
const origLines = md.split('\n')
const backLines = back.split('\n')
console.log('\n=== 逐行对比 ===')
const maxLen = Math.max(origLines.length, backLines.length)
for (let i = 0; i < maxLen; i++) {
  const o = origLines[i] ?? '(无)'
  const b = backLines[i] ?? '(无)'
  if (o !== b) {
    console.log(`行${i + 1}: 原始="${o}" → 往返="${b}"`)
  }
}