...
/**
 * 知识库 AI 编辑 agent 的 Skill 管理 API —— 主对话 {@link com.changy.tailoragent.tool.skill.SkillController}
 * 的<b>独立副本</b>，路由挂在 {@code /api/kb-skills}，操作 {@link KbSkillRegistry}（{@code kb-skills/} 目录）。
 * <p>
 * 导入两条路径与主对话一致：拖拽（{@code POST /import}）与原生文件夹选择（{@code POST /import-dir}）。
 */
@RestController
@RequestMapping("/api/kb-skills")
public class KbSkillController {

    private static final Logger log = LoggerFactory.getLogger(KbSkillController.class);

    private final KbSkillRegistry registry;

    public KbSkillController(KbSkillRegistry registry) {
    ...
    public ApiResponse<List<SkillInfo>> importFiles(@RequestBody ImportRequest req) {
        ...
        try {
            List<KbSkillRegistry.ImportFile> files = new ArrayList<>();
            for (ImportFile f : req.files()) {
                ...
                files.add(new KbSkillRegistry.ImportFile(f.path(), content));
            }
            registry.importFromFiles(req.folderName(), files);
            log.info("已导入知识库 Skill 文件夹: {}({} 个文件)", req.folderName(), files.size());
            return ApiResponse.success(registry.list());
        } catch (IllegalArgumentException e) {
            return ApiResponse.error(e.getMessage());
        } catch (Exception e) {
            log.error("导入知识库 Skill 失败", e);
    ...
    /** 原生文件夹选择对话框(套路同 SkillController#openDirChooser) */
